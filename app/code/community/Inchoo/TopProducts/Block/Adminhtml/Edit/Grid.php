 ...
 */
class Inchoo_TopProducts_Block_Adminhtml_Edit_Grid extends Mage_Adminhtml_Block_Widget_Grid {

    public function __construct() {
        parent::__construct();

        $this->setId('inchoo_top_products');
        $this->setDefaultSort('entity_id');
        $this->setUseAjax(true);

        $this->setRowClickCallback('TopRowClick');
    }
    ...
    protected function _prepareCollection() {
        ...
        if ($action_name == 'exportCsv' || $action_name == 'exportXml') {
            $collection->addAttributeToFilter('inchoo_top_product', array('eq' => true));
        }
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('name', array(
            ...
            'index' => 'name',
            'renderer' => 'topproducts/adminhtml_edit_renderer_name',
        ));
        ...
        $this->addColumn('visibility', array(
            ...
            'index' => 'visibility',
            'filter' => false,
            'renderer' => 'topproducts/adminhtml_edit_renderer_visibility',
        ));
        ...
        $this->addExportType('*/*/exportCsv', Mage::helper('topproducts')->__('CSV'));
        $this->addExportType('*/*/exportXml', Mage::helper('topproducts')->__('Excel XML'));
    ...
    protected function _getSelectedProducts($json = false) {
        $temp = $this->getRequest()->getPost('top_ids');
        $store = $this->_getStore();

        if ($temp) {
            parse_str($temp, $top_ids);
        }

        $_prod = Mage::getModel('catalog/product')->getCollection()
                ->joinAttribute('inchoo_top_product', 'catalog_product/inchoo_top_product', 'entity_id', null, 'left', $store->getId()) //for categry filter
                //->joinField('category_id', 'catalog/category_product', 'category_id', 'product_id = entity_id', null, 'left')
                ->addAttributeToFilter('inchoo_top_product', '1');
                //->addAttributeToFilter('category_id', 8); //for categry filter
        ...
        if ($json == true) {
            ...
            foreach ($products as $key => $value) {
                if ((isset($top_ids[$value])) && ($top_ids[$value] == 0)) {
    ...
    categoryForm = new varienForm('top_edit_form');
    ...
    document.observe("dom:loaded", function() {
        ...
        var everycheckbox = $$("#inchoo_top_products_table tbody input.checkbox");
    ...
    function categorySubmit(url) {
    	...
    	var params = {};
        var fields = $('top_edit_form').getElementsBySelector('input', 'select');   
        
        categoryForm.submit();
    }
    
    function TopRowClick(grid, event)
    {	
		...
		$("in_top_products").value = checkBoxes.toQueryString();
		console.log("Products", checkBoxes);
	   	$gridName.reloadParams = {'top_ids':checkBoxes.toQueryString()};    
    }
...
EndHTML;
    ...
    private function _appendHtml() {
		...
		var checkbox_all = $$("#inchoo_top_products_table thead input.checkbox").first();
        	var everycheckbox = $$("#inchoo_top_products_table tbody input.checkbox");
