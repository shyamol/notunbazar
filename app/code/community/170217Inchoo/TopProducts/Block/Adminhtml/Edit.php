 ...
 */
class Inchoo_TopProducts_Block_Adminhtml_Edit extends Mage_Adminhtml_Block_Widget_Grid_Container {

    protected $_saveButtonLabel = 'Save Top Products';
    protected $_inchooUrl = 'http://inchoo.net';

    public function __construct() {


        $this->_blockGroup = 'topproducts';
        $this->_controller = 'adminhtml_edit';


        $this->_headerText = Mage::helper('adminhtml')->__('Top products');
    ...
    private function _prependHtml() {
    	...
    	<form id="top_edit_form" action="' . $this->getSaveUrl() . '" method="post" enctype="multipart/form-data">
    	<input name="form_key" type="hidden" value="' . $this->getFormKey() . '" />
    		<div class="no-display">
        		<input type="hidden" name="top_products" id="in_top_products" value="" />
    		</div>
		</form>
    	';
    ...
    private function _appendHtml() {
		...
		<div style="text-align:right;">Community version of <a href="' . $this->_inchooUrl . '/ecommerce/magento/top-products-on-magento-frontpage/" target="_blank">Top Products Extension</a></div>
